...
### batch を上げても速くならない（2026-08-31 実測、RTX 4090）

**確認済み:** `max_batch_size` を振って 120 step ずつ測りました。**VRAM が余っていても速度は伸びません。**

| batch | step/s | **examples/s** | frames/s | peak VRAM |
|---:|---:|---:|---:|---:|
| 16（現行） | 7.41 | **117.5** | 134,012 | 1.95 GB |
| 32 | 4.38 | **140.1** | 163,719 | 3.66 GB |
| 64 | 2.49 | **158.4** | 182,272 | 7.10 GB |

**batch を 4 倍にしても処理サンプル数は 1.35 倍**（117.5 → 158.4 ex/s）。GPU はほぼ演算飽和です。
VRAM は batch にほぼ比例するので 24 GB なら 128 も載りますが、**大きい VRAM を買って batch を
上げる戦略は費用に見合いません**。効くのは GPU そのものの速度（4090 は 3090 比 1.44 倍の
11.74 step/s）か、未実装の AMP / bf16 です。
...
**確認済み:** M3 の継続 run の実費 **$2.148** の内訳は GPU $1.157 / **download $0.868** /
storage $0.111 / upload $0.012 でした。**通信が 40% を占めます。**

環境構築だけで torch + CUDA wheel が 8 GB 前後を落とすので、**どの run にも必ず載ります**。
単価は offer ごとに大きく違い、`tools/vast.py search` の `$/TB` 列で確認できます
（実測で 0.3〜4.0 $/TB と 13 倍の開き。今回選んだ offer はさらに高かった）。
**回線速度で offer を選ぶ前に単価を見ること。**
