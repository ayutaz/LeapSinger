...
一方、**音質と話者類似度の評価、Seed-VC との比較、リアルタイム student は未完了**です。したがって「リアルタイム動作」や「Seed-VC より高品質」はまだ確認済みの成果ではありません。**話者類似度は測定手段そのものが未確立**で、手元の x-vector encoder 2 本は同性の歌手を分離できませんでした（[実行計画](svc-plan.md) M4 の「未決」節に較正表）。M4 で報告している target 側の数値は**自己再構成**であり、「別人の声が target に聞こえるか」ではありません。
...
- **話者類似度をどう測るか（M4 ゴール 2 の未達分）。** 枠組みは [`tools/speaker_similarity.py`](../tools/speaker_similarity.py) にありますが、`transformers` の x-vector encoder 2 本は**歌声の同性ペアを分離できません**（同一話者ペアの下位 5% を、別話者・同性ペアの 83.3% が超える）。ECAPA-TDNN 等への差し替えには**依存の追加**が要ります（**要ユーザー判断**）。異性間は分離するので粗い確認にのみ使えます。
...
| **M4 target fine-tune** | **実施済み（話者類似度だけ未測定）**。base から波音リツへ 20,000 step。**target の自己再構成は上限比 94.8% → 98.1% へ改善する一方、未知 source の内容保持は 0.8599 → 0.8359 へ単調に悪化**。事前登録した規則で **`ckpt_010000` を選択**（train loss だけなら 20,000 step を選んでいた）。peak VRAM 2.08 GB |
| M5〜M6 | 未着手 |
