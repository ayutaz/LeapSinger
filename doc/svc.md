...
- **GAN を有効にして base から学習し直すか**（**要ユーザー判断**）。過平滑そのものへの対処で、
残る差に効く可能性がありますが、**base の作り直し**（vast.ai で数時間）になります。
...
| **M5 offline 品質ゲート** | **客観指標は測定完了、blind listening test だけ未実施**。26 clip を両システムで変換して比較。**話者類似度で Seed-VC が上回り**（16 step で 0.4910 対 0.5912）、guard rail が落ちたため**「Seed-VC より良い」とは書けません**。**F0 追従・V/UV・timing は LeapSVC が上**（timing は 16 step で逆転） |
