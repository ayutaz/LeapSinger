...
M5 が要求する客観指標の道具も 4 つとも作りました（timing / CER / 信号品質 / 推論 RTF）。**ただし測定そのものは未実施**で、動作確認に使った数 clip があるだけです。各指標には実測で分かった限界があります（[評価計画](svc-evaluation.md) 4 節）。
...
- **M5 の合格条件をどこに置くか**（**要ユーザー判断**）。道具は揃いましたが、[要件](svc-requirements.md) 7 節の Gate B は「数値閾値は test set 作成後に確定」としています。とくに **CER と信号品質は上限との差でしか読めない**ため、閾値も差で置くことになります。
- **主観評価（blind listening test）の実施条件**（評価者数・clip 数・除外規則）を、M5 開始前に決める必要があります（**要ユーザー判断**）。
...
| **M5 offline 品質ゲート** | **道具の準備のみ完了**。客観指標 6 種の道具が揃い（話者類似度・timing・CER・信号品質・推論 RTF・明るさ）、**すべて上限との差で読む**形にした。**測定本番と Seed-VC との blind comparison は未実施** |
