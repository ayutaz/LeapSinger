...
現時点でモデル、データ契約、学習・評価・単体推論の配線に加え、**実音声から特徴量を作る前処理**（WAV からコマンド 1 本、再実行で bit 一致）と、**23 話者・約 18 時間の multi-singer base 事前学習**まで到達しています。未知の source singer を入れても内容が崩壊しないことを実測しました。実行環境も Python 3.13 と `uv.lock` で固定しています（[実装状況](svc-implementation-status.md) の「実行環境」表）。一方、**target singer への fine-tune、音質と話者類似度の評価、Seed-VC との比較、リアルタイム student は未完了**です。したがって「リアルタイム動作」や「Seed-VC より高品質」はまだ確認済みの成果ではありません。
...
- **base の高域不足にどう対処するか。** 学習継続（3 択の①）は実施し、1 step が −18.1% → **−13.7%**、1/16 step の乖離が 10.8 → **6.0 点**に縮みました。ただし **3 本中 1 本は悪化**しており、clip 数を増やして測り直すのが先です。その後に「さらに継続」「GAN（②）」「M4 へ進む（③）」を判断します（**要ユーザー判断**）。
