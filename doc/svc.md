...
- **話者性を上げる手をどこまで採るか**（**要ユーザー判断**）。原因は**過平滑**と特定済みで
（[実行計画](svc-plan.md) の調査節）、`--num-steps 16` なら**再学習なしで**未知 source の
回復率が 58.8% → 69.4% に上がります（費用 RTF +4.5%、内容 cos −0.017）。**GAN を有効にした
再学習**は原因そのものへの対処ですが、**base の作り直し**になります。
...
| **M5 offline 品質ゲート** | **客観指標は測定完了、blind listening test だけ未実施**。26 clip を両システムで変換して比較。**話者類似度で Seed-VC が上回り**（0.4712 対 0.5912）、事前登録した guard rail が落ちたため**「Seed-VC より良い」とは書けません**。F0 追従（相関 0.9996 / 半音差 0.01）と V/UV は LeapSVC が上 |
