...
| 未 | **base に高域不足が残っています**（変換出力の spectral centroid が上限比 −4〜−26%）。対処は「M3 の後の分岐」の 3 択で**要ユーザー判断** |
...
**残る鈍さはモデル自身のもので、区間によって差があります。**

| 区間 | source | 上限（GT mel 再合成） | 変換 | 差 |
|---|---:|---:|---:|---:|
| 波音リツ 1st_color | 839 Hz | 884 Hz | 651 Hz | **−26%** |
| 波音リツ anywhere | 1213 Hz | 1282 Hz | 1018 Hz | **−21%** |
| 波音リツ ARROW | 738 Hz | 775 Hz | 745 Hz | −4% |
| VocalSet（未知 source） | 841 Hz | 877 Hz | 839 Hz | −4% |
...
| 選択 | 根拠 | 見積もり |
|---|---|---|
| **① 学習を続ける** | eval loss が 30,000 step でも**単調減少中**だった。素直に効く可能性が高い | 30,000 step 追加で約 1 時間・**$0.3 前後**。`--run_name m3_base` の再実行で自動再開 |
...
**推奨:** **① を先に試す**。最も安く、原因の候補（学習不足）を 1 つ潰せます。①で改善しなければ
過平滑の線が濃くなり、②の根拠が強まります。**①②③のどれを選んでも、進む前に
...
- 入力は `log/m3_base/ckpt_030000.pt`（23 話者 / 30,000 step / 134.7 MB）。**別の `run_name` で
  fine-tune し、base を上書きしないこと**（M3 ゴール 2）。target は **spk_id 22**（波音リツ）。
- **base には高域不足が残っています**（M3 進捗節）。fine-tune 前後で
  [`tools/m3_verify.py`](../tools/m3_verify.py) の spectral centroid を比較し、
  **改善したのか持ち越したのかを必ず記録**します。M4 の similarity 指標だけを見ていると、
  この軸は素通りします。
