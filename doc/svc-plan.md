...
**確認済み:** **M0 / M1 / M2 / M3 は完了**、**M4 も実施済み**（話者類似度だけ未測定）。M5 以降は未着手です。
...
**未測定:** ゴール 2 の**話者類似度**は測れていません（上の「未決」節。encoder が歌声で
較正を通らないため）。ここでの target 側の数値は**自己再構成**であって、
「別人の声がリツに聞こえるか」ではありません。
...
`transformers` にある x-vector モデル 2 本を、素材を変えて 3 通り較正しました。最も公平な
条件（同一技法・別の曲の抜粋）でも、同性では差 0.059 に対しばらつきが 0.08〜0.11 あり、
**同一話者ペアの下位 5% を別話者・同性ペアの 83.3% が超えます**。異性間
（0.5199 対 0.8307）は明確に分かれるので、**性別が違う場合の粗い確認にだけ**使えます。

**歌声で較正を通る encoder に差し替えるまで、同性間の target similarity を報告しません。**
候補は ECAPA-TDNN などですが**依存の追加が要る**ため、環境を固定したまま fine-tune を
回してから着手します。encoder は `embed` 引数で差し替えられるので、較正のやり直しだけで済みます。
