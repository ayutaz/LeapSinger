...
| 未 | **音質を評価していない。** 内容保持・F0 追従・明るさ・話者類似度は測りましたが、それは音質ではありません。**主観評価（blind listening test）は未実施** |
...
**要ユーザー判断（M5 を始める前に決めること）:**

1. **合格とする差の大きさ。** CER と信号品質は上限との差でしか読めないので、閾値も差に置きます。
2. **主観評価の実施条件**（評価者数・clip 数・除外規則）。**実験の前に決めること**
   （M4 の checkpoint 選択規則と同じ理由）。
3. **Seed-VC と上限を共有できるか。** mel 表現もボコーダーも違うので「GT mel の再合成」が
   同じ意味になりません。共通の上限を使うか、上限に依存しない指標（timing・話者類似度）で
   比べるかを決めます。
